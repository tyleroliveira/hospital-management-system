...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    req.session.save(() => res.json({ user: userData, message: 'You are now logged in!' }));
  } catch (err) {
    res.status(400).json(err);
  }
  //res.render("home",{patient: userData})
});
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    req.session.save(() => res.json({ user: patientData, message: 'You are now logged in!' }));
  } catch (err) {
    res.status(400).json(err);
  }
